...
1. 在resource/netease_item_res中增加json文件，如：
    ...
    ```python
    {
        "minecraft:item": {
            ...
            "components": {
                "minecraft:icon": "shield"//空字符则会已盾牌模型显示
            }
        },
        "format_version": "1.10"
    }
...
1. resource/models/entity中自定义自己的盾牌模型，在制作模型时，若你想直接复用原版动画和模型，模型的所有cube需放在名为shield的bone下，
   由于盾牌放在左右手的模型位置是不同的，若你不打算制作两种模型。可在shield骨骼下写"binding": "q.item_slot_to_bone_name(c.item_slot)"（可在demo的盾牌模型文件中看见，模型文件format_version需要大于等于1.16.0），
